...
const Dropdown: React.FC<DropdownProps> = ({ Toggle, MobileTitle, placement, children }) => {
    ...
    return (
        <div className="relative">
            ...
            <div className={`${menu ? "max-h-dvh sm:p-1" : "max-h-0 max-sm:delay-500"} ${placement === "right" ? "sm:right-0" : ""} font-default max-sm:top-0 max-sm:left-0 fixed sm:absolute max-sm:z-50 sm:z-30 sm:mt-2 sm:bg-white max-sm:w-screen max-sm:h-dvh flex items-end overflow-hidden sm:[box-shadow:0_0_4px_0_#00000080] sm:rounded-[4px]`}>
