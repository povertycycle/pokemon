...
const Navigator: React.FC = () => {
    ...
    return (
        <div className="z-[1] flex flex-col py-1 text-[1.25rem] gap-1 relative shrink-0 w-[40px]">
            {
                Object.entries(Tab).map((entries: [string, Tab], i: number) => (
                    <div key={i} className="flex items-center h-[36px] cursor-pointer" onClick={() => { tab !== entries[1] && setTab(entries[1]) }}>
                        <div className={`peer/navs z-[1] relative aspect-square h-full flex items-center justify-center rounded-r-[4px] hover:rounded-r-[0px] transition-colors ${tab === entries[1] ? "bg-base-white border-y-2 border-r-2 text-base-red-dark" : "bg-base-red-dark text-base-white"}`}><i className="ri-question-mark" /></div>
