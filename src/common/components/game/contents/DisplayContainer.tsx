...
type DisplayContainerProps = {
    tab: Tab | null;
    returnToSelection: (source: Tab) => void;
}
        ...
        <div className="w-full h-full relative z-0">
            {
                (() => {
                    switch (tab) {
                        ...
                        case Tab.Berries:
                            return <BerryDatabase back={returnToSelection} />
                        case Tab.Items:
                        case Tab.Evolution:
                        case Tab.Machines:
                        case Tab.Moves:
                        case Tab.Calculator:
                            return (
                                <div className="w-screen text-center font-default flex-col h-screen flex items-center justify-center bg-white">
                                    ...
                                    <button className="absolute right-2 top-2 hover:brightness-90 transition-[filter] flex items-center justify-center px-2 sm:px-4 rounded-[4px] gap-4 text-white" onClick={() => { returnToSelection(tab); }} style={{ background: TAB_COLORS[tab] }}>
