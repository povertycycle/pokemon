...
const SearchBar: React.FC = () => {
    ...
    const filterByName = (value: string) => {
        ...
        if (!!value && pools.current) {
            for (let i = 0; (i < pools.current.length); i++) {
                if (pools.current[i].name.toLowerCase().includes(value.toLowerCase())) {
                    if (_data.length === 5) {
                        _hasMore = true;
                        break;
                    }
    ...
    return (
        <div className="sm:py-4 sm:order-1 flex flex-col relative max-sm:h-[32px] sm:h-full z-[2] max-sm:grow sm:w-full">
                ...
                <div className={`z-[2] flex-col bg-base-white sm:w-[400px] rounded-[8px] max-sm:w-screen translate-y-2 sm:-translate-y-2 sm:shadow-xl overflow-hidden`}>
                    ...
                    }
                    {
                        ...
                        <div className="w-full sm:py-1 text-[1rem] sm:text-[1.125rem] flex items-center justify-center">
                            <a className="hover:text-base-red-dark" target="_blank" href={`/?${DESTINATION}=database&${TAB_SELECTION}=pokemon&${NAME_QUERY}=${valueRef.current}`}>
                                {
                                    data.length === 0 ?
                                        "No Pokemon Found" :
                                        "See More"
                        ...
                        <div className={`text-white h-full flex z-[2] relative w-full sm:hover:bg-black/0 bg-black/25 cursor-pointer`}>
                            ...
                            <div className="w-full h-full flex flex-col justify-end py-[6px] relative">
                                ...
                                <div className="flex text-[1.25rem] items-center">
                                    ...
                                    <div className="flex ml-2 text-[0.75rem] leading-4 text-black gap-1 pt-[3px]">
                                        {
                                            (data.name?.replace(data.species, "").split("-").slice(1)).map((id: string, i: number) => (
                                                <div className={`h-fit px-2 flex items-center justify-center bg-base-white-soft rounded-[12px]`} key={i}>
                                                    {id.toUpperCase()}
                                                </div>
                                            ))
                                        }
