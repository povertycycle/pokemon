            ...
            <div className={`fixed z-[1] left-0 top-0 text-[1.25rem]`}>
                {
                    Object.entries(Tab).map((entries: [string, Tab], i: number) => (
                        <div className={`${styles.entryAnimation} group/selector absolute flex items-center transition-all duration-300 cursor-pointer ${!tab ? "overflow-hidden border-2" : "left-[-40px] hover:left-[0]"}`} style={{
                            ...
                            <div className={`relative duration-300 z-[0] px-2 h-full flex items-center justify-center transition-[transform,colors] ${COLORS[i].border} ${!tab ? `translate-x-[-100%] shrink-0 w-full text-[4rem] translate-y-[20%] ${COLORS[i].text}` : `${tab === entries[1] ? "bg-base-white" : "bg-base-white-soft"} text-base border-y-2 border-r-2 translate-x-[-100%] group-hover/selector:translate-x-0 rounded-r-[4px] ${COLORS[i].text}`}`}>{entries[0]}</div>
                        </div>
                    ))
                }
            </div>
