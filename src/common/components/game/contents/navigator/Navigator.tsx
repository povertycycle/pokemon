...
import { BerryTabIcon, ItemTabIcon, MachineTabIcon, PokemonTabIcon, UnknownTabIcon } from "../Icons";
...
const Navigator: React.FC = () => {
    const COLORS = [
        ...
        { bg: "bg-x-dark", text: "text-x-dark", border: "border-base-white-dark", transition: "group-hover/selector:translate-y-[-105%]" },
            ...
            <div className={`fixed z-[1] left-0 top-0 text-[1.25rem]`}>
                {
                    Object.entries(Tab).map((entries: [string, Tab], i: number) => (
                        <div className={`${styles.entryAnimation} group/selector absolute flex items-center transition-all duration-400 cursor-pointer ${!tab ? "overflow-hidden border-2" : "left-[-40px] hover:left-[0]"}`} style={{
                            ...
                            <div className={`shrink-0 z-[2] relative h-full flex items-center justify-center transition-all duration-400 overflow-hidden ${!tab ? "w-full" : `${COLORS[i].bg} aspect-square rounded-r-[4px] hover:rounded-r-[0px]`}`}>
                                <div className={`${tab && tab !== entries[1] ? "bg-black/25" : ""} z-[2] absolute w-full h-full`} />
                                <div className={`z-[1] relative duration-400 transition-[transform,padding] max-h-[192px] h-full w-full flex items-center justify-center text-base-white ${!tab ? "text-[5rem] group-hover/selector:translate-y-[-30%] p-8" : `p-1`}`}>
                                    {Icons[entries[1]]}
                                </div>
                            </div>
                            <div className={`${!tab ? `${COLORS[i].transition} ${COLORS[i].bg}` : `${tab && tab !== entries[1] ? "opacity-25" : "opacity-0"}" bg-black rounded-r-[4px]`} h-full w-full top-0 absolute z-[1] transition-transform duration-400`} />
                            <div className={`relative duration-400 z-[0] px-2 h-full flex items-center justify-center transition-[transform,colors] ${COLORS[i].border} ${!tab ? `translate-x-[-100%] shrink-0 w-full text-[4rem] translate-y-[20%] ${COLORS[i].text}` : `${tab === entries[1] ? "bg-base-white" : "bg-base-white-soft"} text-base border-y-2 border-r-2 translate-x-[-100%] group-hover/selector:translate-x-0 rounded-r-[4px] ${COLORS[i].text}`}`}>{entries[0]}</div>
                        </div>
                    ))
                }
            </div>
