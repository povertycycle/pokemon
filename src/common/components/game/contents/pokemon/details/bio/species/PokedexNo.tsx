...
const PokedexNo: React.FC<PokedexNoProps> = ({ pokedex_numbers }) => {
    ...
    return (
        <div className={`flex flex-col shrink-0 h-fit shadow-[-2px_4px_10px_2px_#00000080] border-x-2 border-b-2`} style={{ borderColor: palette[0] }}>
            ...
            <div className={`flex flex-col max-h-[256px] text-base font-mono tracking-[-0.5px] transition-height overflow-x-hidden overflow-y-auto z-[1] ${styles.overflowWhite}`} style={{ color: colors[1] }}>
                {
                    pokedex_numbers.map((pdn: Entry, i: number) => (
                        <div className={`flex gap-4 items-center justify-between px-2 shrink-0 h-[24px] py-[2px] ${i % 2 === 0 ? "" : "bg-black/10"}`} key={i}>
