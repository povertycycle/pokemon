...
const Training: React.FC<TrainingProps> = ({ capture_rate,
    ...
    const { darker, bgFont } = { darker: palette[0], bgFont: isDark(palette[0]) ? "white" : "black" };
    const { lighter, brFont } = { lighter: palette.at(-1) ?? "", brFont: isDark(palette.at(-1)) ? "white" : "black" }

    return (
        <div className="flex flex-col border-2 rounded-[8px] overflow-hidden w-fit h-fit p-1" style={{ color: bgFont, borderColor: lighter }}>
...
const CaptureRate: React.FC<{ capture_rate: number }> = ({ capture_rate }) => {
    const calculate = Math.pow((Math.floor(4096 / 3 * capture_rate) / 1044480), 0.75);
...
const GenderRate: React.FC<{ gender_rate: number, male: string, female: string }> = ({ gender_rate, male, female }) => {
    const mRatio = (8 - gender_rate) / 8 * 100;
    const fRatio = gender_rate / 8 * 100;
    ...
    return (
        <div className="px-2 flex h-full grow justify-between text-[1.125rem] leading-6 items-center">
            ...
            <div className="w-full h-[14px] rounded-[50px] overflow-hidden flex border-2" style={{ borderColor: gender_rate < 0 ? "black" : male }}>
                {
                    gender_rate < 0 ?
...
const EggGroups: React.FC<{ egg_groups: string[] }> = ({ egg_groups }) => {
    return (
        <div className="flex flex-col gap-[2px] justify-between w-full">
            <div className="underline h-full text-[1.125rem] flex items-center justify-center leading-6 text-center pb-[2px]">Egg Group{egg_groups.length > 1 ? "s" : ""}</div>
