...
const Navigation: React.FC<NavigationProps> = ({ sprites, active, setActive }) => {
    ...
    const font = isDark(palette.at(-1)) ? "white" : "black";
    const lFont = isDark(palette[0]) ? "white" : "black";

    return (
        <div className={`relative z-[1] h-fit w-[112px] max-h-full flex flex-col gap-2 py-2 pl-2 pr-[6px] rounded-l-[16px] overflow-hidden shadow-base-black`} style={{ background: palette[0] }}>
            {
                sprites &&
                Object.keys(sprites.versions).concat([OTHERS]).map((gen: string, i: number, ref: string[]) => {
                    ...
                    return (
                        <div key={i} className={`${i === 0 ? "rounded-tl-[10px]" : ""} ${i === ref.length - 1 ? "rounded-bl-[10px]" : ""} ${active === gen ? "px-2" : "hover:translate-x-[4px] px-[6px]"} transition-transform w-full h-[28px] flex items-center justify-end overflow-hidden leading-4 cursor-pointer`}
                            style={{ background: active !== gen ? palette[0] : palette.at(-1), border: active !== gen ? `2px solid ${palette.at(-1)}` : "", color: active !== gen ? lFont : font }} onClick={pickGen}>
