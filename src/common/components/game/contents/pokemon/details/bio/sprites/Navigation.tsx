...
const Navigation: React.FC<NavigationProps> = ({ sprites, active, setActive }) => {
    ...
    return (
        <div className={`text-base relative z-[1] transition-height w-[112px] max-h-full flex flex-col py-2 pl-2 pr-[6px] rounded-l-[12px] bg-black/50 border-y-2 border-l-2 overflow-hidden shadow-base-black`} style={{ borderColor: palette[0], height: h, gap: `${GAP}px` }}>
            {
                sprites &&
