...
const Expander: React.FC = () => {
    ...
    const expand = () => {
        setExpanded(prev => {
            ...
            if (display) display.style.width = prev ? "75%" : "100%";
            if (nav) nav.style.width = prev ? "25%" : "0%";
            return !prev;
        });
    }

    return (
        <div className={`z-[2] absolute h-full bottom-0 transition-[left] duration-500 ${expanded ? "left-full" : "left-[75%]"}`}>
