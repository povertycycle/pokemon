...
const TypeFilter: React.FC<TypeFilterProps> = ({ filter, toggle }) => {
    ...
    return (
        <div ref={ref} className="flex items-center justify-between w-full text-[1.125rem] leading-[1.125rem] rounded-full transition-colors" style={{ background: `linear-gradient(90deg,${firstColor},${secondColor})` }}>
...
const Dropdown: React.FC<DropdownProps> = ({ type, setType, parentRef, pool, left }) => {
    ...
    return (
        <div className={`flex items-center select-none relative cursor-pointer w-full h-full`}>
            <div className={`h-full w-full flex items-center justify-center gap-2 ${color}`} onClick={openMenu}>
                {
                    type ?
