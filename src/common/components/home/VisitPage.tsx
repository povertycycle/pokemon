const VisitPage: React.FC<{ inside: boolean }> = ({ inside }) => {
    ...
    return (
        <a href={PROFILE} target="_blank" className={`${inside ? "top-[calc(100%-0.25rem-36px)]" : "top-[0.25rem]"} left-[0.25rem] z-[5] h-[36px] w-[36px] hover:w-[256px] bg-base-red-dark text-base-white border-base-white border-2 rounded-[4px] absolute transition-[top,width] overflow-hidden flex gap-2 duration-500`}>
