...
export async function validatePokemonDatabase(): Promise<string | null> {
    return fetch(BASE_API_URL_POKEMON).then(res => {
        ...
        return updateValidator(res, Stores.Pokemon);
    })
}
...
function fetchPokemonData(id: string, species: string): Promise<PokemonDetails | null> {
    return new Promise(result => {
        Promise.all([
